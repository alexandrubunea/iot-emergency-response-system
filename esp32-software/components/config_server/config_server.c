...
static esp_err_t config_handler(httpd_req_t* req) {
	...
	const cJSON* sound = cJSON_GetObjectItem(root, "sound");
	const cJSON* gas = cJSON_GetObjectItem(root, "gas");
	const cJSON* fire = cJSON_GetObjectItem(root, "fire");

	if (api_key == NULL || ssid == NULL || password == NULL || motion == NULL || sound == NULL ||
		gas == NULL || fire == NULL) {
...
httpd_handle_t config_server_init(config_t* config) {
	...
	httpd_uri_t config_uri = {
		.uri = "/api/config", .method = HTTP_POST, .handler = config_handler, .user_ctx = config};

	httpd_uri_t check_uri = {
		.uri = "/api/check", .method = HTTP_GET, .handler = check_handler, .user_ctx = NULL};
...
void wait_for_configuration(u_int8_t seconds_between_checks) {
	while (!esp32_config_received) {
		ESP_LOGI(TAG, "Configuration wasn't received yet. Checking again in %d seconds.",
