    ...
    if (event_base == WIFI_EVENT) {
        switch (event_id) {
            case WIFI_EVENT_AP_STACONNECTED:
                ESP_LOGI(TAG, "Device connected to ESP32.");
                break;
            case WIFI_EVENT_AP_STADISCONNECTED:
                ESP_LOGI(TAG, "Device disconnected from ESP32.");
                break;
            case WIFI_EVENT_STA_CONNECTED:
                ESP_LOGI(TAG, "Connected to local network.");
                break;
            case WIFI_EVENT_STA_DISCONNECTED:
                ESP_LOGI(TAG, "Disconnected from local network.");
                break;
        }
    }
}
...
static wifi_status wifi_config_sta_mode(char* sta_ssid, char* sta_pass) {
...
static wifi_status wifi_config_ap_mode(char* ap_ssid, char* ap_pass) {
...
wifi_status wifi_init(wifi_mode_t mode, char* sta_ssid, char* sta_pass, char* ap_ssid, char* ap_pass) {
    if (wifi_initialize() == WIFI_STATUS_ERROR)
        return WIFI_STATUS_ERROR;
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error setting WiFi mode: %s", esp_err_to_name(err));
        return WIFI_STATUS_ERROR;
    }
