...
void app_main(void) {
    ...
    if (boot_sequence(&device_cfg) != ESP_OK)
        return;
...
esp_err_t boot_sequence(config_t** device_cfg) {
    ...
    *device_cfg = allocate_configuration();
    ...
    /* Init wifi_manager */
    if (wifi_init(wifi_mode, (*device_cfg)->ssid, (*device_cfg)->password, WIFI_AP_SSID, WIFI_AP_PASS) == WIFI_STATUS_ERROR)
        goto error;
...
free_resources:
    ...
    if (freshly_configured) {
        ...
        ESP_LOGI(TAG, "This device was freshly configured and will reboot in %d seconds...", WAIT_TIME_BEFORE_REBOOT);
...
config_t* allocate_configuration() {
    ...
    if (config == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for ESP32 configurtion.");
        return NULL;
    }
